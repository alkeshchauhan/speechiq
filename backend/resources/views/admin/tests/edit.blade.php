<x-admin-layout>
    ...
    <div class="max-w-xl mx-auto">
        <div class="bg-slate-950/40 backdrop-blur border border-slate-800 rounded-2xl p-6">
            <form method="POST" action="{{ route('admin.tests.update', $test->id) }}" class="space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <label class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Test Title</label>
                    <input type="text" name="title" value="{{ old('title', $test->title) }}" required class="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-indigo-500 transition duration-200" placeholder="e.g. Cambridge Pronunciation Test">
                    @error('title')
                ...
                <div>
                    ...
                    <select name="type" required class="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-indigo-500 transition duration-200">
                        <option value="READ_ALOUD" {{ old('type', $test->type) === 'READ_ALOUD' ? 'selected' : '' }}>Read Aloud Paragraph Practice</option>
                        <option value="AI_INTERVIEW" {{ old('type', $test->type) === 'AI_INTERVIEW' ? 'selected' : '' }}>Interactive AI Mock Interview</option>
                    </select>
                    @error('type')
                        <p class="text-xs text-rose-400 mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <textarea name="description" rows="4" class="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-indigo-500 transition duration-200" placeholder="Describe what the test evaluates...">{{ old('description', $test->description) }}</textarea>
                    @error('description')
                        <p class="text-xs text-rose-400 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center justify-between">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ $test->is_active ? 'checked' : '' }}>
