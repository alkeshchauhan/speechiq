...
def analyze_audio_speechbrain_librosa(audio_bytes: bytes, target_text: str = None, transcript: str = "") -> dict:
    ...
    try:
        ...
        logger.error(f"DSP Audio analysis failed: {e}")
...
async def read_aloud_analyze(
    ...
    use_gemini = request.headers.get("X-Use-Gemini", "0") == "1"
    openai_model = request.headers.get("X-OpenAI-Model", "gpt-4o-mini")
    gemini_model = request.headers.get("X-Gemini-Model", "gemini-1.5-flash")
...
async def speech_to_text(
    ...
    if gemini_key:
        try:
            prompt = "Transcribe this audio recording exactly. Return only the transcription text, with no extra commentary."
            ...
            logger.error(f"Error in speech_to_text with Gemini API: {e}. Falling back to mock transcription.")
...
async def interview_analyze(
    ...
    tone = "Professional"
    feedback = "Your response is articulate and structured. Pacing is natural, and vocabulary is sound."
...
1. Evaluate the grammar quality and assign a score (0-100).
2. Evaluate vocabulary richness and assign a score (0-100).
3. Evaluate communication skills and logic of the answer, and assign a score (0-100) to "communication_score".
4. Identify the tone of the response (e.g., "Professional", "Informal", "Academic", "Confident", "Hesitant").
5. Provide constructive and detailed feedback about their response, highlighting style, structure, vocabulary, and grammar. Keep it under 100 words.
...
"""
            ...
            vocabulary_score = int(data.get("vocabulary_score", 75))
            communication_score = int(data.get("communication_score", 75))
            tone = data.get("tone", "Professional")
            feedback = data.get("feedback", "Articulate response.")
            
        except Exception as e:
            logger.error(f"Error analyzing interview with Gemini API: {e}. Falling back to default scores.")
    ...
    full_language = lang_map.get(language, language.title()) if language else "English"

    overall_score = int((grammar_score + vocabulary_score + communication_score + confidence_score + pronunciation_score + fluency_score) / 6)
    if is_gibberish:
        overall_score = min(9, overall_score)
...
"""
            ...
            logger.error(f"Error generating question with Gemini API: {e}. Falling back to default list.")

    # Simple logic to cycle follow-up questions
    history_len = len(req.history) if req.history else 0
    if history_len == 0:
        question = "To begin the interview, could you tell me about yourself and your primary technical stack?"
    elif history_len == 1:
        question = "That sounds impressive. How do you approach scaling applications and handling load spikes?"
    elif history_len == 2:
        question = "Could you describe your testing methodology? How do you ensure code coverage and robustness before production?"
    else:
        question = "Finally, how do you handle conflicts or design disagreements within a software engineering team?"
...
Provide a summary feedback paragraph assessing their performance, clarity, and overall technical expertise shown. Keep the feedback under 100 words.
"""
            ...
            logger.error(f"Error generating feedback with Gemini API: {e}. Falling back to mock feedback.")
